                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php if($review['product_image']): ?>
                                            <img src="<?= BASE_URL ?>public/uploads/<?= $review['product_image'] ?>" width="50" class="img-thumbnail">
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <?php if($review['reply_content']): ?>
                                            <div class="alert alert-secondary p-2 mt-2 mb-0">
                                                <small><i class="fas fa-reply me-1"></i> <strong>Phản hồi:</strong> <?= htmlspecialchars($review['reply_content']) ?></small>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td>
        ...
        <div class="modal fade" id="replyModal<?= $review['id'] ?>" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    <?php 
                        // Nếu controller bên Staff chưa có hàm reply, bạn cần bổ sung hoặc trỏ tạm về admin nếu muốn dùng chung logic xử lý (nhưng sẽ redirect về admin)
                        // Tốt nhất là tạo hàm reply trong Staff/ReviewController.php
                        $replyAction = ($prefix == 'staff') ? 'reply' : 'replyReview'; 
                        // Lưu ý: Tên hàm bên AdminController bạn đặt là 'replyReview', bên StaffController nên đặt giống hoặc chỉnh lại ở đây.
                        // Giả sử bên Admin là replyReview, ta dùng logic này:
                        $methodName = 'replyReview'; 
                    ?>
                    
                    <form action="<?= BASE_URL . $prefix ?>/review/<?= $methodName ?>/<?= $review['id'] ?>" method="POST">
                        ...
                        <div class="modal-body">
                            <div class="form-group mb-3">
                                <label class="fw-bold mb-2">Nội dung trả lời:</label>
                                <textarea name="reply_content" class="form-control" rows="4" required placeholder="Nhập câu trả lời của shop..."><?= $review['reply_content'] ?></textarea>
                            </div>
                        </div>
