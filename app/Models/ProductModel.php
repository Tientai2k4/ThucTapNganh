...
class ProductModel extends Model {
    ...
    public function addVariant($productId, $size, $color, $stock, $price = 0, $salePrice = 0) {
        $sql = "INSERT INTO product_variants (product_id, size, color, stock_quantity, price, sale_price) 
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($sql);
        // Lưu ý: "issidd" -> i: int, s: string, d: double/decimal
        $stmt->bind_param("issidd", $productId, $size, $color, $stock, $price, $salePrice);
        return $stmt->execute();
    }

    // [MỚI] Cập nhật thông tin biến thể
    public function updateVariant($variantId, $size, $color, $stock, $price = 0, $salePrice = 0) {
        $sql = "UPDATE product_variants SET size = ?, color = ?, stock_quantity = ?, price = ?, sale_price = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("ssiddi", $size, $color, $stock, $price, $salePrice, $variantId);
        return $stmt->execute();
    }
    ...
    public function getVariantsDetail($variantIds) {
        ...
        // Lấy cả giá của Variant (v_price) và giá của Product cha (p_price)
        $sql = "SELECT pv.id as variant_id, pv.size, pv.color, pv.product_id, 
                    pv.price as v_price, pv.sale_price as v_sale_price,
                    p.name, p.price as p_price, p.sale_price as p_sale_price, p.image 
        ...
        // Xử lý Logic ưu tiên giá
        foreach ($rows as &$row) {
            // Nếu Variant có set giá (>0) thì dùng, ngược lại dùng giá chung của Product
            $row['price']      = ($row['v_price'] > 0) ? $row['v_price'] : $row['p_price'];
            $row['sale_price'] = ($row['v_sale_price'] > 0) ? $row['v_sale_price'] : $row['p_sale_price'];
        }
        return $rows;
    }
